  ...
  return (
    <LayoutToExperiments domain='React' title='useMemoExample'>
      ...
      <NameInputAddress
        name={name}
        address={address}
        onChangeName={(e: { target: { value: SetStateAction<string> } }) => setName(e.target.value)}
...
function ThemeYeah({ name }: { name: string }) {
  ...
  function handleClick() {
    setTheme(
      theme === 'text-black bg-zinc-50' ? 'text-white bg-zinc-700' : 'text-black bg-zinc-50'
    );
  }
  return (
...
const GreetingTheme = memo(function GreetingTheme({ name }: { name: string }) {
  console.log('Greeting Theme was rendered at', new Date().toLocaleDateString());
